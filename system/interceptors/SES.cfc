		...
		<cfscript>
			...
			if( findnocase("handler", arrayToList(arguments.routeParams)) ){
				...
				for(x=1; x lte listLen(rString,"/"); x=x+1){
					...
					//if (isSimpleValue(extRoot)) {( len(extRoot) AND directoryExists(extRoot & "/" & foundPaths & thisFolder) )}
					if( directoryExists(root & "/" & foundPaths & thisFolder)
						OR
						( isSimpleValue(extRoot) AND len(extRoot) AND directoryExists(extRoot & "/" & foundPaths & thisFolder) )
						){
						/* Save Found Paths */
					...
					else if (isArray(extRoot)) {
						for(i=1; i lte arrayLen(extRoot); i=i+1){
							if (directoryExists(extRoot[i] & "/" & foundPaths & thisFolder)) {
								/* Save Found Paths */
								foundPaths = foundPaths & thisFolder & "/";
								/* Save new Event */
								if(len(newEvent) eq 0){
