	...
	<cffunction name="locatePluginPath" access="private" returntype="string" hint="Locate a full plugin instantiation path from the requested plugin name" output="false" >
		...
		<cfscript>
			...
			if ( arguments.custom OR len(arguments.module) ){
				...
				if ( isSimpleValue(getCustomPluginsExternalPath()) ){
				...
				} else if ( isArray(getCustomPluginsExternalPath()) ){
					for(i=1;i LTE ArrayLen(getCustomPluginsExternalPath());i=i+1){
						extpluginpath =  getCustomPluginsExternalPath()[i] & "/" & pluginFilePath;
						// if the file exists in this plugin path
						if ( fileExists( extpluginpath ) or fileExists( "/" & extpluginpath ) ){
							return "#getCustomPluginsExternalPath()[i]#.#arguments.plugin#";
						}
					}
				}
