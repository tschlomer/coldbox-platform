...
<!------------------------------------------- PUBLIC ------------------------------------------->
	...
	<cffunction name="getRegisteredHandler" access="public" hint="I parse the incoming event string into an event handler bean that is used for executions." returntype="coldbox.system.beans.EventHandlerBean"  output="false">
		...
		if( find(":", arguments.event) ){
			...
			if( handlerIndex ){
				//if we have an array of external locations, set the invocation path for the handler if it exists.
				if (isArray(controller.getSetting("HandlersExternalLocation"))){
					for(i=1;i LTE ArrayLen(controller.getSetting("HandlersExternalLocation"));i=i+1){
						handlerFile = ExpandPath("/" & replace(controller.getSetting("HandlersExternalLocation")[i],".","/","all"));
						handlerFile = handlerFile & "/" & handlerReceived & ".cfc";
						if (fileexists(handlerFile)) {
							HandlerBean.setInvocationPath(controller.getSetting("HandlersExternalLocation")[i]);
							break;
						}
					}
				} else {
	...
	<cffunction name="registerHandlers" access="public" returntype="void" hint="I register your application's event handlers" output="false">
		...
		if( isSimpleValue(HandlersExternalLocationPath) and len(HandlersExternalLocationPath) ){
		...
		} else if ( isArray(HandlersExternalLocationPath) and arrayLen(HandlersExternalLocationPath) ) {
			for(i=1;i LTE ArrayLen(HandlersExternalLocationPath);i=i+1){
				//Check for Handlers Directory Location
				if ( not directoryExists(HandlersExternalLocationPath[i]) ){
					getUtil().throwit("The external handlers directory: #HandlersExternalLocationPath[i]# does not exist please check your application structure.","","HandlerService.HandlersDirectoryNotFoundException");
				}
				//Get recursive Array listing
				extHandlers = getHandlerListing(HandlersExternalLocationPath[i]);
				for(n=1;n LTE ArrayLen(extHandlers);n=n+1) {
					ArrayAppend(HandlersExternalArray,extHandlers[n]);
				}
			}
		}
		//Verify it
